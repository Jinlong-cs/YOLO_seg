...
def build_parser():
    ...
    parser.add_argument(
        "--mapper-script",
        help="Deprecated compatibility argument. It is ignored because YOLO_seg now manages config and calibration data internally.",
    )
    return parser
...
def collect_calibration_images(args):
    from yolo_seg.calibration import collect_images_from_dir, collect_images_from_split, sample_images
...
def run_quantize(args):
    from yolo_seg.calibration import write_calibration_tensors
    from yolo_seg.rdk_x5_config import write_config
    ...
    if args.mapper_script:
        print("--mapper-script is deprecated and ignored. YOLO_seg now generates config.yaml and calibration data internally.")
